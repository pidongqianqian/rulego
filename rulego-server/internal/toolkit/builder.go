...
// Validate 验证配置文件
func (v *Validator) Validate(configFile string) error {
	// 检查文件是否存在
...
// Upgrade 升级配置文件
func (u *Upgrader) Upgrade(configFile, targetVersion string) error {
	// 检查文件是否存在
	if _, err := os.Stat(configFile); os.IsNotExist(err) {
		return fmt.Errorf("配置文件不存在: %s", configFile)
	}
	...
	// 更新版本
	cfg.Server.Version = targetVersion
...
// NewServer 创建新的服务器管理器
func NewServer() *Server {
	return &Server{}
}

// Start 启动服务器
func (s *Server) Start(configFile string) error {
	// 构建服务器
	cmd := exec.Command("go", "build", "-o", "bin/rulego-server", "./cmd/server")
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("构建服务器失败: %v", err)
	}

	// 启动服务器
	cmd = exec.Command("./bin/rulego-server", "-config", configFile)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
